...
export async function getBusinessMetrics(
  ...
  try {
    const { data, error } = await supabase.rpc('get_business_metrics_admin', {
      days_back: daysBack
    });
...
export async function getAggregatedMetrics(): Promise<{
  ...
  try {
    const { data, error } = await supabase.rpc('get_aggregated_metrics_admin');
...
export async function getUsersAnalytics(
  ...
  try {
    const { data, error } = await supabase.rpc('get_users_analytics_admin', {
      limit_count: limit
    });
...
export async function getRPETrends(
  ...
  try {
    const { data, error } = await supabase.rpc('get_rpe_trends_admin', {
...
/**
 * Ottieni programmi più popolari
 */
export async function getProgramPopularity(): Promise<{ data: ProgramPopularity[] | null; error: any }> {
  try {
    const { data, error } = await supabase.rpc('get_program_popularity_admin');
