...
import type { PoseLandmarks, PoseLandmark } from '../../types/biomechanics.types';
...
export interface IssueSegmentMap {
  ...
  correctionParams?: {
    /** Angolo target in gradi (per rotate_around_point) */
    targetAngle?: number;
    /** Punto pivot per rotazione */
    pivot?: LandmarkKey;
    /** Endpoint da ruotare */
    endpoint?: LandmarkKey;
    /** Catena di punti da raddrizzare (per straighten_chain) */
    chain?: LandmarkKey[];
    /** Punto di riferimento per la proiezione (per project_to_vertical_plane) */
    referenceLine?: [LandmarkKey, LandmarkKey];
    /** Punto da proiettare */
    projectPoint?: LandmarkKey;
    /** Asse di simmetria (per symmetrize) */
    symmetryAxis?: [LandmarkKey, LandmarkKey];
    /** Punti da simmetrizzare (coppie left/right) */
