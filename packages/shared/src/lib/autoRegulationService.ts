 ...
 * Initialize with initAutoRegulationService(supabaseClient) before use.
 */
...
/**
 * Initialize the auto-regulation service with a Supabase client
 * Must be called before using any service functions
 */
export function initAutoRegulationService(client: SupabaseClient): void {
  supabase = client;
}
...
function getSupabase(): SupabaseClient {
  if (!supabase) {
    throw new Error('[AutoRegulation] Supabase client not initialized. Call initAutoRegulationService first.');
  }
  return supabase;
}
...
export async function logWorkout(
  ...
  try {
    ...
    const { data: workoutLog, error: createError } = await getSupabase()
      .from('workout_logs')
      .insert({
        user_id: userId,
    ...
    const { error: exerciseError } = await getSupabase()
      .from('exercise_logs')
...
export async function addExerciseLog(
  ...
  // Trigger automatico aggiornera session_rpe in workout_logs
...
export async function analyzeRPE(
  ...
  try {
    ...
    const { data, error } = await getSupabase().rpc('get_avg_rpe_last_sessions', {
      p_user_id: userId,
...
export async function getExercisesNeedingAdjustment(
  ...
  try {
    const { data, error } = await getSupabase().rpc('get_exercises_needing_adjustment', {
      p_user_id: userId,
      p_program_id: programId,
      p_sessions_count: sessionsCount
    });
...
export async function applyAutoRegulation(
  ...
  try {
    ...
    const adjustment: ProgramAdjustment = {
      ...
      adjustment_type: analysis.adjustment_type as 'decrease_volume' | 'increase_volume' | 'deload_week',
    ...
    const { data: savedAdjustment, error: saveError } = await getSupabase()
    ...
    // L'utente decidera se accettare, rifiutare o modificare tramite DeloadSuggestionModal
