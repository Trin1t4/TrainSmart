...
export function calculateCorrectedSegments(
  ...
  const { correctionStrategy, correctionParams, errorSegments } = mapping;
...
function calcProjectToVerticalPlane(
  ...
  // Proietta x del punto sulla linea tra refFrom e refTo
  // Per KNEE_VALGUS: il ginocchio dovrebbe avere x sulla linea hip-ankle
  ...
  // Ricostruisci i segmenti con il punto corretto
  const result: CorrectedSegment[] = [];

  // Per ogni segmento di errore, se contiene il projectPoint, sostituiscilo
  ...
  // Per issues bilaterali (left/right), calcola anche il lato opposto
...
/**
 * Calcola la proiezione per il lato opposto (bilaterale)
 */
function calcProjectBilateral(
  ...
  // Se il punto originale e' left_, calcola anche right_ e viceversa
  ...
  // Trova la reference line opposta
  ...
  // Ricostruisci i segmenti opposti
 ...
 * Usato per EXCESSIVE_FORWARD_LEAN e ELBOW_FLARE.
 */
function calcRotateAroundPoint(
  ...
  // Angolo target (relativo alla verticale)
...
function calcProjectToGround(
  ...
  // Il suolo e' la y massima tra heel e foot_index
  ...
  for (const seg of mapping.errorSegments) {
    ...
    // Proietta i punti heel al suolo
 ...
 * Usato per SPINE_FLEXION e LOWER_BACK_ROUND: spine neutra = linea retta hip->shoulder.
 */
function calcStraightenChain(
  ...
  // Se ci sono segmenti bilaterali, aggiungi anche il lato destro
 ...
 * Usato per WEIGHT_SHIFT_LATERAL.
 */
