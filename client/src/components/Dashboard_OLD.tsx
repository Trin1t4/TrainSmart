...
export default function Dashboard() {
  ...
  try {
    // Check se il programma è stato appena generato
    const recentProgram = localStorage.getItem('programGenerated');
    if (recentProgram) {
      const { timestamp } = JSON.parse(recentProgram);
      const minutesAgo = (Date.now() - new Date(timestamp).getTime()) / 60000;
      
      if (minutesAgo < 5) {
        console.log('Programma generato di recente, skip check');
        setHasProgram(true);
        setLoading(false);
        return;
      }
  ...
  async function handleGenerateProgram() {
    ...
    try {
      ...
      if (onboardingData.goal === 'motor_recovery') {
        ...
        // 🧠 SISTEMA INTELLIGENTE - CALCOLA IL LEVEL CORRETTAMENTE
        let userLevel = 'beginner';
        
        // Prima prova dallo screening (più affidabile)
        if (screeningData.level) {
          userLevel = screeningData.level;
          console.log('📊 Level from screening:', userLevel);
        } 
        // Altrimenti calcola dal quiz score
        else if (quizData && quizData.score !== undefined) {
          ...
          console.log(`📊 Calculated level from quiz score ${quizData.score}: ${userLevel}`);
        }
        // Se c'è già un level nel quiz, usalo
        else if (quizData && quizData.level) {
          userLevel = quizData.level;
          console.log('📊 Level from quiz data:', userLevel);
        }
        
        // 🎯 MAPPING INTELLIGENTE DEI GOAL ITALIANI
        const intelligentGoalMap = {
          'forza': 'strength',
          'massa': 'muscle_gain',
        ...
        const originalGoal = onboardingData.goal || 'muscle_gain';
        const mappedGoal = intelligentGoalMap[originalGoal.toLowerCase()] || originalGoal;
        
        // 📊 LOGGING DETTAGLIATO PER DEBUG
        console.group('🧠 INTELLIGENT SYSTEM ANALYSIS');
        console.log('Quiz Data:', { 
          score: quizData?.score, 
          level: quizData?.level 
        });
        console.log('Screening Data:', { 
          level: screeningData?.level,
          finalScore: screeningData?.finalScore 
        });
        ...
        programInput = {
          ...
          equipment: onboardingData.equipment || {},
          goal: mappedGoal,  // USA IL GOAL MAPPATO INTELLIGENTEMENTE
          level: userLevel,  // USA IL LEVEL CALCOLATO INTELLIGENTEMENTE
      ...
      console.log('🎯 FINAL DATA FOR GENERATION:', {
...
// Salva in localStorage per evitare loop
